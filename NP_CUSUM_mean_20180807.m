...
rcal = 1000;
no = [3000];                     % Reference sample size         n = #(X) = [30 50 100 200 500 1000 3000 5000 10000];  
mo = [1 3];                       % Batch sample size to monitor  m = #(Y) = [1 3 5 7 10]
...
for m = mo              %%%%%%%%% MONITORING SAMPLE SIZE
    ...
    for iu = 1:uu
        ...
        for R =1:REP  % I can't use parfor here, because I'm saving ARL(j).
            ...
            h = bootstrap_calibration(X, n, m, arl, rcal, 1, 0, k);
            H = h*sqrt(m*n*(n+m+1)/12);
            for r=1:rep  %%% Parallel toolbox 
                [RL, var, count] = bootstrap_CUSUM_ReferenceSample_Li(X,n,m,H,k,1,mu2,RL,var);
            end % parfor(rep) ARL;
